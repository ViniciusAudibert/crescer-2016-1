...
SELECT PR.NOME FROM PRODUTO PR WHERE NOT EXISTS(SELECT 1 FROM PEDIDOITEM PI WHERE PI.IDPRODUTO = PR.IDPRODUTO);
...
SELECT P.DATAPEDIDO,PR.NOME,PI.QUANTIDADE,PI.PRECOUNITARIO,PI.PRECOUNITARIO * PI.QUANTIDADE AS PRECOTOTAL FROM PEDIDO P JOIN CLIENTE C ON C.IDCLIENTE = P.IDCLIENTE JOIN PEDIDOITEM PI ON PI.IDPEDIDO = P.IDPEDIDO JOIN PRODUTO PR ON PR.IDPRODUTO = PI.IDPRODUTO WHERE C.IDCLIENTE = :pIDCLIENTE; 
...
SELECT TO_CHAR(P.DATAPEDIDO, 'MON-YYYY') AS DATAPEDIDO, COUNT(DISTINCT PI.IDProduto ) AS PRODUTOS_DISTINTOS ,SUM(P.VALORPEDIDO) AS VALOR_PEDIDO ,COUNT(1) AS QUANTIDADETOTOALPRODUTOS, MIN(P.VALORPEDIDO) AS MENORVALOR, MAX(P.VALORPEDIDO) AS MAIORVALOR, TO_CHAR(AVG(P.VALORPEDIDO), 'fm9999999.90') AS PRECOMEDIO FROM PEDIDO P JOIN PEDIDOITEM PI ON P.IDPEDIDO = PI.IDPEDIDO GROUP BY TO_CHAR(P.DATAPEDIDO, 'MON-YYYY');