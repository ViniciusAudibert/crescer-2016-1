...
$(function() {
  ...
  function informacoesCavaleiros(i){
    ...
    for(var key in goldSaints[i]){
      if(key !== 'imagens'){
        if(key === 'dataNascimento'){
          var date = new Date(goldSaints[i][key]);
          var dd = date.getDate();
  ...
  // Exercicio Adicionar imagens
  function exibirCavaleiros(i){
    $(this).hide();
    var value = goldSaints[i];
    var $imagem = $('<img>').attr('src', value.imagens[0].url).attr('alt',value.nome);
    $imagem[0].onload = function(){
      $('#imagem-cavaleiros').append($('<li>').append((($imagem).attr('id',i).attr('class','cavaleiro-info'))))
    };
    setTimeout(function() {
      $imagem.off('mouseleave');
    }, 5000);
    if (i < goldSaints.length - 1) exibirCavaleiros(i + 1);

  };

  // Exercicio formulario
  function frmNovoCavaleiro(){
    var $frmNovoCavaleiro = $('#frm-novo-cavaleiro');
    $frmNovoCavaleiro.submit(function(e) {
      console.log($frmNovoCavaleiro.serialize());
      var nome = formData.get('nome');
      var urlImagem = formData.get('url');
      var tipoSanguineo = formData.get('tipo-sanguineo');
      var dataNascimento = formData.get('data');
      var altura = formData.get('altura');
      var peso = formData.get('peso');
      var signo = formData.get('signo');
      var localNascimento = formData.get('nascimento');
      var localTreinamento = formData.get('treinamento');
      var golpes = formData.get('golpes');

      var $imagem = $('<img>').attr('src', urlImagem).attr('alt',nome);
      $imagem
      $('#imagem-cavaleiros').append($('<li>').append((($imagem).attr('id',goldSaints.length).attr('class','cavaleiro-info'))));
    ...
    $('#exibe-cavaleiros').click(function() {
      exibirCavaleiros(0);
    });
    ...
    $('body').on('mouseout','.cavaleiro-info', function(){
